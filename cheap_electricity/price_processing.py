...
def process_and_categorize_prices(
    prices_data: Dict[str, Any]) -> Tuple[Optional[Price], Optional[Price]]:
  ...
  hourly_prices = [
      p for p in all_hourly_prices
      if _normalize(p.get('geo_name', '')) == 'peninsula'
  ]
  ...
  for price_info in hourly_prices:
    ...
    price_objects.append(
        Price(hour=price_time, value=value, unit="€/MWh", category=cat))

  now = datetime.datetime.now()
  current_hour = now.hour
  previous_hour = (current_hour - 1) % 24
  current_price = next(
      (p for p in price_objects if p.hour.hour == current_hour), None)
  previous_price = next(
      (p for p in price_objects if p.hour.hour == previous_hour), None)

  if not current_price or not previous_price:
    print(
        f"Price not found for the current or previous hour ({current_hour}:00)."
    )
    return None, None
